...
class FirstCoverLetter extends StatefulWidget {
  final String code;
  final GetAllCover? cover;

  const FirstCoverLetter({Key? key, required this.code, this.cover})
      : super(key: key);

  @override
  _FirstCoverLetterState createState() => _FirstCoverLetterState();
}

class _FirstCoverLetterState extends State<FirstCoverLetter> {
  ...
  void initState() {
    ...
    if (widget.code == "update") {
      fetchData().then((data) {
        ...
        setState(() {
          globalController.firstName.value =
              data['data']['nameAndContact']['firstName'] ?? "Null Null";
          globalController.lastName.value =
              data['data']['nameAndContact']['lastName'] ?? "Null Null";

          globalController.email.value =
              data['data']['nameAndContact']['email'] ?? 'no email';
          globalController.phoneNumber.value = data['data']['nameAndContact']
                  ['phoneNumber'] ??
              'no phone number';
          globalController.city.value =
              data['data']['nameAndContact']['city'] ?? 'no phone number';
          globalController.state.value =
              data['data']['nameAndContact']['state'] ?? 'no phone number';
          globalController.profession.value =
              data['data']['nameAndContact']['profession'] ?? 'no profession';
          globalController.zip.value =
              data['data']['nameAndContact']['zip'] ?? 'no profession';

          globalController.rFirstName.value =
              data['data']['recipient']['firstName'] ?? "Null Null";
          globalController.rLastName.value =
              data['data']['recipient']['lastName'] ?? "Null Null";

          globalController.rEmail.value =
              data['data']['recipient']['email'] ?? 'no email';
          globalController.rPhoneNumber.value =
              data['data']['recipient']['phoneNumber'] ?? 'no phone number';
          globalController.rCity.value =
              data['data']['recipient']['city'] ?? 'no phone number';
          globalController.rState.value =
              data['data']['recipient']['state'] ?? 'no phone number';
          globalController.rZip.value =
              data['data']['recipient']['zip'] ?? 'no profession';
          globalController.companyName.value =
              data['data']['recipient']['companyName'] ?? 'no profession';

          globalController.linkedin.value =
              data['data']['socialLinks']['linkedin'] ?? 'no profession';
          globalController.github.value =
              data['data']['socialLinks']['github'] ?? 'no profession';
          globalController.portfolio.value =
              data['data']['socialLinks']['portfolio'] ?? 'no profession';

          globalController.date.value = data['data']['date'] ?? 'no profession';
          globalController.subject.value =
              data['data']['subject'] ?? 'no profession';
          globalController.greeting.value =
              data['data']['greeting'] ?? 'no profession';
          globalController.opening.value =
              data['data']['opening'] ?? 'no profession';
          globalController.letterBody.value =
              data['data']['letterBody'] ?? 'no profession';
          globalController.callToAction.value =
              data['data']['callToAction'] ?? 'no profession';
          globalController.closing.value =
              data['data']['closing'] ?? 'no profession';
          globalController.signature.value =
              data['data']['signature'] ?? 'no profession';
          globalController.availability.value =
              data['data']['availability'] ?? 'no profession';
          globalController.confidentiality.value =
              data['data']['confidentiality'] ?? 'no profession';
          globalController.gaps.value = data['data']['gaps'] ?? 'no profession';
          globalController.relocation.value =
              data['data']['relocation'] ?? 'no profession';
          globalController.salaryRequirements.value =
              data['data']['salaryRequirements'] ?? 'no profession';
          globalController.tempID.value =
              data['data']['tempID'] ?? 'no profession';
        });
      }).catchError((error) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          if (widget.code == "update") {
            ...
            String message = await PostResume()
                .updateCover(globalController, widget.cover!.sId);
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
  ...
  <div id="main-content">
    <div id="left-column">
      ...
      <div class="letter-body">
        <p>
          ...
          ${globalController.companyName.value}, as advertised on [where you found
          the job posting, e.g., LinkedIn or the company\'s website]. I am excited
          about the opportunity to contribute my skills and experience to your team.
          ...
        </p>
      </div>
      <p>Best Regards</p>
      <p onclick="showDialog()">${globalController.signature}</p>
    ...
    <div id="right-column">
      ...
      <div id="personal-info">
        <p id="address">
          Address: ${globalController.city} ${globalController.state} ${globalController.zip}
        </p>
        <p id="phone">Phone: ${globalController.phoneNumber}</p>
        <p id="email">Email: ${globalController.email}</p>
      </div>
    </div>
  </div>
</body>
      ...
      """);
        ...
        child: _isLoading
            ...
            : Icon(widget.code == "update" ? Icons.update : Icons.done),
      ),
      body: SafeArea(
        top: true,
        child: _isFetching
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                height:
                                    MediaQuery.of(context).size.height * 0.852,
                                      ...
                                      GestureDetector(
                                        ...
                                        child: Padding(
                                          padding: EdgeInsetsDirectional.fromSTEB(
                                              0, 5, 0, 0),
                                          child: Text(
                                            'phone: ${globalController.phoneNumber}',
                                            style: TextStyle(
                                                fontWeight: FontWeight.w500),
                                          ),
                                        ),
