...
class _VerifyEmailScreenState extends State<VerifyEmailScreen>  with TickerProviderStateMixin{
  ...
  Future<String> verify(String email, String otp) async {
    ...
    try {
      final apiUrl = Uri.parse('https://lizmyresume.onrender.com/user/auth/verify');
      final headers = {
  ...
  Future<String> resendOTP()async{
    ...
    try {
      final apiUrl = Uri.parse('https://lizmyresume.onrender.com/user/auth/resendOtp');
      final headers = {
        'Content-Type': 'application/json',
      };
      ...
      if (response.statusCode == 200) {
        ...
        return "OTP resent"; // Data posted successfully
      } else {
        setState(() {
          _isLoading = false;
        });
            ...
            child: FadeTransition(
              ...
              child: Column(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: ()async{
                     String message = await resendOTP();
                  ...
                  GestureDetector(
                    onTap: ()async{
                      if(_otpController.text.isEmpty){
                        ...
                        String message = await verify(widget.email, _otpController.text.trim());
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                                message),
